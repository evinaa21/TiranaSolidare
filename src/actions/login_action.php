...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!validate_csrf_token($_POST['_csrf_token'] ?? '')) {
        header('Location: ' . ts_app_path('views/login.php?error=csrf_expired'));
        exit();
    }

    // Rate limiting: max 5 login attempts per 15 minutes
    if (!check_rate_limit('login', 5, 900)) {
        header('Location: ' . ts_app_path('views/login.php?error=rate_limited'));
        exit();
    }
    ...
    if (empty($email) || empty($password)) {
        header('Location: ' . ts_app_path('views/login.php?error=empty_fields' . ($redirect ? '&redirect=' . urlencode($redirect) : '')));
        exit();
    }
    ...
    if ($user && password_verify($password, $user['fjalekalimi'])) {
        ...
        if ($accountStatus === 'blocked') {
            header('Location: ' . ts_app_path('views/login.php?error=account_blocked' . ($redirect ? '&redirect=' . urlencode($redirect) : '')));
            exit();
        }
        if ($accountStatus === 'deactivated') {
            header('Location: ' . ts_app_path('views/login.php?error=account_deactivated' . ($redirect ? '&redirect=' . urlencode($redirect) : '')));
            exit();
        }

        $activationErrorKey = ts_guardian_consent_error_key($user);
        if ($activationErrorKey !== null) {
            header('Location: ' . ts_app_path('views/login.php?error=' . $activationErrorKey . ($redirect ? '&redirect=' . urlencode($redirect) : '')));
            exit();
        }
        ...
        header('Location: ' . ts_app_path('views/login.php?error=wrong_credentials' . ($redirect ? '&redirect=' . urlencode($redirect) : '')));
        exit();
    }
} else {
